...
SELECT FECHA_CONTABLE,DECODE(PROYECTO,'EK BALAM','CANTARELL', PROYECTO)AS ACTIVO, CONCEPTO, DET_CONCEPTO, SUM(VOLUMEN)
FROM RDP2.PRON_JUST
WHERE
PRON_JUST.FECHA_CONTABLE BETWEEN TO_DATE (TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'YEAR')) 
                                                 AND  LAST_DAY(TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'YEAR')+364)
       AND PRON_JUST.PROYECTO IN ('CANTARELL','EK BALAM')
       GROUP BY FECHA_CONTABLE,PROYECTO, CONCEPTO, DET_CONCEPTO