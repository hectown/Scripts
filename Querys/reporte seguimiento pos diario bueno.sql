SELECT PRON_JUST.FECHA_CONTABLE,
 ...
 WHERE PRON_JUST.FECHA_CONTABLE BETWEEN TO_DATE (TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'MONTH'))
       ...
       AND PRON_MOVS.ID_INSTALACION = POZOS.ID AND DET_CONCEPTO LIKE 'CIERRE DE POZOS POR ALTA RGA'
   ...
   SELECT TRUNC(TO_DATE(:FECHA,'DD/MM/YYYY')+ROWNUM),'CANTARELL',NULL,NULL,NULL,NULL,NULL,NULL,NULL
FROM DUAL 
CONNECT BY ROWNUM < (SELECT TO_NUMBER((LAST_DAY(TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'YEAR')+364)) - TO_DATE(:FECHA,'DD/MM/YYYY')) 
FROM DUAL)+1



SELECT TRUNC(TO_DATE(:FECHA,'DD/MM/YYYY')+ROWNUM),'CANTARELL',NULL,NULL,NULL,NULL,NULL,NULL,NULL,TO_CHAR(TRUNC(TO_DATE(:FECHA,'DD/MM/YYYY')+ROWNUM),'MON')
FROM DUAL 
CONNECT BY ROWNUM < (SELECT TO_NUMBER((LAST_DAY(TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'YEAR')+364)) - TO_DATE(:FECHA,'DD/MM/YYYY')) 
FROM DUAL)+1
 ...
 PRON_JUST.FECHA_CONTABLE BETWEEN TO_DATE (TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'YEAR'))
                                                 AND TO_DATE (:FECHA,'DD/MM/YYYY')
                                                 
                                                 
SELECT TO_DATE (TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'YEAR')) FROM DUAL

SELECT LAST_DAY(TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'YEAR')+364) FROM DUAL


SELECT TO_DATE (TRUNC (TO_DATE (:FECHA,'DD/MM/YYYY'),'MONTH')) FROM DUAL
